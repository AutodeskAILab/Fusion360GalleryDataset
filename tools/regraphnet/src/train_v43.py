...
def train_test(graph_pairs_formatted,args):
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model.state_dict(),'../ckpt/model_v43.ckpt')
        # test
        model.eval()
        loss,acc0,acc1,acc2=0,[0,0],[0,0],[0,0]
        with torch.no_grad():
...
if __name__=="__main__":
    ...
    model=NodePointer(nfeat=graph_pairs_formatted[0][1].size()[1],nhid=args.hidden,dropout=args.dropout,Use_GCN=False)
