...
def format_graph_data(data):
    ...
    pt_xs,pt_ys,pt_zs=features_Poi[:,0::3],features_Poi[:,1::3],features_Poi[:,2::3]
    span_x,span_y,span_z=np.max(pt_xs)-np.min(pt_xs),np.max(pt_ys)-np.min(pt_ys),np.max(pt_zs)-np.min(pt_zs)
...
def train_test(graph_pairs_formatted,args):
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model.state_dict(),'../ckpt/model_v41.ckpt')
        # test
        model.eval()
        loss,acc0,acc1,acc2=0,[0,0],[0,0],[0,0]
        with torch.no_grad():
