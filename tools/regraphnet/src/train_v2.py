...
def load_dataset(args):
    ...
    for k in tqdm(range(len(seqs))):
        ...
        for step in data_seq['sequence']:
            ...
            graph_pairs_formatted.append([adj_tar,features_tar,adj_cur,features_cur,label_start_now,label_end_now,label_action_now])
            counter[0]+=1
...
def train(graph_pairs_formatted,args):
    ...
    for epoch in range(args.epochs):
        ...
        print('Epoch: {:04d}'.format(epoch+1),'loss: {:.4f}'.format(loss/acc0[1]),'start: {:.3f}'.format(acc0[0]/acc0[1]*100.0),'end: {:.3f}'.format(acc1[0]/acc1[1]*100.0),'op: {:.3f}'.format(acc2[0]/acc2[1]*100.0))

if __name__=="__main__":
    # args
    parser=argparse.ArgumentParser()
    parser.add_argument('--no-cuda',action='store_true',default=False,help='Disables CUDA training.')
    parser.add_argument('--dataset',type=str,default='RegraphPerFace_02',help='Dataset name.')
